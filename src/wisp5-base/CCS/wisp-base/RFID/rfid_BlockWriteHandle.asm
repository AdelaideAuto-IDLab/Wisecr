...
%{
	...
	//IEM removed, for command refractory please switch to master branch
%}
...
; 		to byte aligned (e.g. by overriding the MB value) -- This would also mean the actual data
;		would also be byte aligned saving many instructions.
calc_wordPtr:
...
; Merge upper and lower bytes ((upper & 0x7F) << 7) | (lower & 0x7F))
	...
	rpt #7					; TODO: Check if a sequence of RLC.B and RLC.W operations is faster than this.
	rlax.w 	R_scratch0
...
store_regular:
	...
	;AND.B	#0x11, R_scratch2								;[1] Offset &= 0x11
...
; Check if handle matches.
	...
	JNE     checkEavesDropping                              ;[2] Handle doesn't match, so skip respond stuff and check if i am eavesdropping.
...
;	RETA
	...
	JMP		update_authentication_state

checkEavesDropping:
	TST.B	(rfid.observerMode)
	JZ		exit_safely
...
update_control_message:
	...
	JMP 	move_timing_delay_BlockWrite
...
; Don't bother waiting if we are in observerMode, since we don't want to ACK.
	TST.B	(rfid.observerMode)
	JNZ		exit_safely

; Just in case the user has no BlockWrite callback, we delay the response.
move_timing_delay_BlockWrite:
;	BIS.B	#PIN_AUX3, &PDIR_AUX3
;	BIS.B	#PIN_AUX3, &PIN_AUX3_OUT
	MOV     #1, R_scratch0                                ;[2] Why is this value working for both readers? WTF happens during T5 anyway!?
