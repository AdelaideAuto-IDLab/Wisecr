...
uint16_t ADC_read(void) {
    PJOUT |= BIT1;  //switch on the high side converter
    __delay_cycles(16); // give time for MOS to response
    ADC_asyncRead(0);

    while (ADC_isBusy()){__no_operation();};
    ADC_SM.lastValue = ADC12MEM0;
    PJOUT &= ~BIT1; //turn off the high side converter
