...
inline void epcInit(void) {
    ...
    wispData.epcBuf[9] = veri;        // Tag firmware version
...
#ifdef TEMP_FIRM_BUG
        // A possible bug in the temperature firmware, the bits are shifted the wrong way!
        wispData.epcBuf[7] = (adc_temperature >> 0) & 0xFF;
        wispData.epcBuf[8] = (adc_temperature >> 8) & 0xFF;
#else
